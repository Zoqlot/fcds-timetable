    ...
    <>
                          ...
                          {rows.map((row: any) => {
                            ...
                            return (
                              <button
                                ...
                                className={`w-full text-left p-3 rounded-2xl border flex items-start gap-3 transition-all ${invalid ? "bg-zinc-100 border-zinc-200 opacity-60 cursor-not-allowed" : selected ? "bg-violet-50 border-violet-300 ring-1 ring-violet-200 shadow-sm" : "bg-white border-zinc-200 hover:border-violet-300 hover:shadow-sm"}`}
                              >
                                ...
                                <div className="min-w-0 flex-1">
                                  <div className="flex justify-between gap-3">
                                    <span className="text-[10px] font-black uppercase tracking-widest text-violet-600">
                                      {row.source_code || "MATH0 / ELEC"}
                                    </span>
                                    <span className="text-[10px] font-black text-zinc-400">
                                      {credits} cr
                                    </span>
                                  </div>
                                  <div className={`text-sm font-black leading-tight mt-1 ${selected ? "text-violet-900" : "text-zinc-800"}`}>
                                    {row.courses?.name ?? row.source_name}
                                  </div>
                                </div>
                              </button>
                            );
      ...
      <main className="flex-1 h-full flex flex-col p-6 overflow-hidden bg-zinc-100 print:bg-white print:p-0">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {generatedSchedules.length > 0 && uniqueDayCounts.length > 1 && (
              <div className="flex items-center bg-white border border-zinc-300 rounded-xl p-1 shadow-sm h-9">
                ...
                <button 
                  onClick={() => { setDayFilter('all'); setCurrentIndex(0); }} 
                ...
                {uniqueDayCounts.map(count => (
                  <button 
                    key={count} 
                    onClick={() => { setDayFilter(count); setCurrentIndex(0); }} 
        ...
        {loading ? (
          ...
          <div className="flex-1 min-h-0"><TimetableGrid timetable={displayedSchedules[currentIndex] || displayedSchedules[0] || []} theme={theme} onRemove={() => {}} /></div>
        )}
      </main>
    </>
  );
